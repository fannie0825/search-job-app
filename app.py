...
def display_job_card(result, index):
    ...
    with col2:
        if job['url'] != '#':
            st.link_button("Apply Now →", job['url'], use_container_width=True, type="primary")

def main():
    st.markdown('<h1 class="main-header">🔍 Semantic Job Search</h1>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">AI-powered job matching</p>', unsafe_allow_html=True)
